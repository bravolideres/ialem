interface IconProps {
  size?: number;
  className?: string;
  /** Solid fill color — when set, replaces the default gold gradient */
  fill?: string;
}

/**
 * The IAlem brand icon / symbol (the stylised "IA" mark used as favicon).
 * Renders at `size × size` with the brand gradient fill by default,
 * or a solid `fill` colour when provided.
 */
export default function IAlemIcon({ size = 34, className, fill }: IconProps) {
  const useSolid = !!fill;

  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="-2 -2 308 196"
      width={size}
      height={size}
      role="img"
      aria-label="IAlem"
      className={className}
      style={{ display: "block" }}
    >
      {!useSolid && (
        <defs>
          <linearGradient id="icon-grad" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stopColor="#C9A46B" />
            <stop offset="100%" stopColor="#A78655" />
          </linearGradient>
        </defs>
      )}
      <path
        fill={useSolid ? fill : "url(#icon-grad)"}
        fillRule="evenodd"
        d="M83.7 14.1c-3.6 7.4-14.2 29.4-23.7 48.9-30.2 62.1-35.9 74-48.1 99.3-6.6 13.6-11.7 25.2-11.4 25.7 1 1.6 32 1.3 33.3-0.3 1-1.2 37.9-77.2 59-121.2 25.3-53.1 30.3-63.8 29.7-64.3-0.3-0.3-7.7-0.7-16.5-1l-16-0.5-6.3 13.4z M176.6 3.7c-2.9 5.8-71.3 144.7-80.2 163.1-5.2 10.6-9.4 20-9.4 20.7 0 1.3 2.9 1.5 17.8 1.5l17.7 0 43-28.1c23.7-15.4 49.1-32 56.5-36.8 7.4-4.9 18.8-12.3 25.3-16.5 6.4-4.2 11.7-8.1 11.7-8.8 0-1.6-37.8-78.9-40.1-81.9-1.6-2.2-1.8-2-9.3 13.2-4.2 8.5-7.6 16.4-7.6 17.6 0 2.4 4.3 12.5 14.5 33.8 3.7 7.7 7.1 15 7.6 16.2 0.9 2.5 1.2 2.3-49.1 29.6-26.9 14.5-33 17.5-33 16.3 0-0.4 37.2-76.5 63.4-129.6l5.7-11.5-5.8-0.6c-3.2-0.4-10.6-0.7-16.5-0.8l-10.7-0.1-1.5 2.7z M251.6 117c-6 3.8-11.5 7.5-12.3 8.2-1 1.1 0.4 5 8.6 22.3 14.2 30.4 18.8 39.3 21 40.5 2.3 1.2 32.7 1.4 33.5 0.2 0.5-0.9-31.9-67.8-36.1-74.5-1.3-2-2.7-3.7-3.1-3.6-0.4 0-5.6 3.1-11.6 6.9z"
      />
    </svg>
  );
}

